...
BAR.recoilRecoverySpeed = 1
BAR.aimFovTp = 30
BAR.aimFovFp = 30

local renderables =
{
	"$CONTENT_DATA/Tools/Renderables/BAR/BAR_Base.rend",
	"$CONTENT_DATA/Tools/Renderables/BAR/BAR_Anim.rend"
}

local renderablesTp =
{
	"$CONTENT_DATA/Tools/Renderables/BAR/char_BAR_anims_tp.rend",
	"$CONTENT_DATA/Tools/Renderables/BAR/BAR_offset_tp.rend"
}

local renderablesFp =
{
	"$CONTENT_DATA/Tools/Renderables/BAR/char_BAR_anims_fp.rend",
	"$CONTENT_DATA/Tools/Renderables/BAR/BAR_offset_fp.rend",
...
function BAR:client_initAimVals()
	local cameraWeight, cameraFPWeight = self.tool:getCameraWeights()
	self.aimWeight = math.max( cameraWeight, cameraFPWeight )
end

function BAR:server_onCreate()
...
function BAR:client_onCreate()
	...
	mgp_toolAnimator_initialize(self, "BAR")

	self.network:sendToServer("server_requestAmmo")
end

function BAR.client_onDestroy(self)
	mgp_toolAnimator_destroy(self)
end

function BAR.client_onRefresh( self )
	self:loadAnimations()
end

function BAR.loadAnimations( self )
	...
	if self.cl_isLocal then
		self.fpAnimations = createFpAnimations(
			self.tool,
			{
				equip = { "BAR_pickup", { nextAnimation = "idle" } },
				unequip = { "BAR_putdown" },

				idle = { "BAR_idle", { looping = true } },
				shoot = { "BAR_shoot", { nextAnimation = "idle" } },
				...
				aimInto = { "BAR_aim_into", { nextAnimation = "aimIdle" } },
				aimExit = { "BAR_aim_exit", { nextAnimation = "idle", blendNext = 0 } },
				aimIdle = { "BAR_aim_idle", { looping = true } },
				aimShoot = { "BAR_aim_shoot", { nextAnimation = "aimIdle"} },

				sprintInto = { "BAR_sprint_into", { nextAnimation = "sprintIdle",  blendNext = 0.2 } },
				sprintExit = { "BAR_sprint_exit", { nextAnimation = "idle",  blendNext = 0 } },
				sprintIdle = { "BAR_sprint_idle", { looping = true } },
			}
		)
	end
...
function BAR:client_updateAimWeights(dt)
	-- Camera update
	local bobbing = 1
	if self.aiming then
...
function BAR:server_spendAmmo(data, player)
...
function BAR:sv_n_trySpendAmmo(data, player)
	local v_owner = self.tool:getOwner()
...
function BAR.client_onUpdate( self, dt )
...
function BAR:client_onEquip(animate, is_custom)
...
function BAR:client_onUnequip(animate, is_custom)
	if not is_custom and TSU_IsOwnerSwimming(self) then
		return
	end
...
function BAR:sv_n_onAim(aiming)
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function BAR:cl_n_onAim(aiming)
	if not self.cl_isLocal and self.tool:isEquipped() then
		self:onAim(aiming)
	end
end

function BAR:onAim(aiming)
	self.aiming = aiming
...
function BAR:cl_n_onShoot(dir)
	if not self.cl_isLocal and self.tool:isEquipped() then
		self:onShoot(dir)
	end
end

function BAR:onShoot(dir)
...
function BAR:calculateFirePosition()
...
function BAR:calculateTpMuzzlePos()
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function BAR:calculateFpMuzzlePos()
...
function BAR:sv_n_onReload(anim_id)
	self.network:sendToClients("cl_n_onReload", anim_id)
end

function BAR:cl_n_onReload(anim_id)
	if not self.cl_isLocal and self.tool:isEquipped() then
		self:cl_startReloadAnim(id_to_anim_name[anim_id])
	end
end

function BAR:cl_startReloadAnim(anim_name)
...
function BAR:sv_n_checkMag()
	self.network:sendToClients("cl_n_checkMag")
end

function BAR:cl_n_checkMag()
	local s_tool = self.tool
...
function BAR:cl_startCheckMagAnim()
...
function BAR:client_onToggle()
	if not self:client_isGunReloading() and not self.aiming and not self.tool:isSprinting() and self.fireCooldownTimer == 0.0 and self.equipped then
		if self.ammo_in_mag > 0 then
			...
			sm.gui.displayAlertText("BAR: No Ammo. Reloading...", 3)
