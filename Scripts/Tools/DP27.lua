...
DP27.recoilRecoverySpeed = 1
DP27.aimFovTp = 30
DP27.aimFovFp = 30

local renderables =
{
	"$CONTENT_DATA/Tools/Renderables/DP27/DP27_Base.rend",
	"$CONTENT_DATA/Tools/Renderables/DP27/DP27_Anim.rend"
}

local renderablesTp =
{
	"$CONTENT_DATA/Tools/Renderables/DP27/char_DP27_anims_tp.rend",
	"$CONTENT_DATA/Tools/Renderables/DP27/DP27_offset_tp.rend"
}

local renderablesFp =
{
	"$CONTENT_DATA/Tools/Renderables/DP27/char_DP27_anims_fp.rend",
	"$CONTENT_DATA/Tools/Renderables/DP27/DP27_offset_fp.rend",
...
function DP27:client_onCreate()
	...
	mgp_toolAnimator_initialize(self, "DP27")

	self.network:sendToServer("server_requestAmmo")
end

function DP27.client_onDestroy(self)
	mgp_toolAnimator_destroy(self)
end

function DP27.client_onRefresh( self )
	self:loadAnimations()
end

function DP27.loadAnimations( self )
	self.tpAnimations = createTpAnimations(
		self.tool,
		{
			...
			reload_empty = { "DP27_tp_empty_reload", { nextAnimation = "idle", duration = 1.0 } },
			reload = { "DP27_tp_reload", { nextAnimation = "idle", duration = 1.0 } },
			ammo_check = { "DP27_tp_ammo_check", {nextAnimation = "idle", duration = 1.0}}
		}
	)
	...
	if self.cl_isLocal then
		self.fpAnimations = createFpAnimations(
			self.tool,
			{
				equip = { "DP27_pickup", { nextAnimation = "idle" } },
				unequip = { "DP27_putdown" },
				...
				ammo_check = { "DP27_ammo_check", { nextAnimation = "idle", duration = 1.0 } },

				aimInto = { "DP27_aim_into", { nextAnimation = "aimIdle" } },
				aimExit = { "DP27_aim_exit", { nextAnimation = "idle", blendNext = 0 } },
				aimIdle = { "DP27_aim_idle", { looping = true } },
				aimShoot = { "DP27_aim_shoot", { nextAnimation = "aimIdle"} },

				sprintInto = { "DP27_sprint_into", { nextAnimation = "sprintIdle",  blendNext = 0.2 } },
				sprintExit = { "DP27_sprint_exit", { nextAnimation = "idle",  blendNext = 0 } },
				sprintIdle = { "DP27_sprint_idle", { looping = true } },
			}
		)
	end
...
function DP27:client_updateAimWeights(dt)
	-- Camera update
	local bobbing = 1
	if self.aiming then
...
function DP27:server_spendAmmo(data, player)
...
function DP27:sv_n_trySpendAmmo(data, player)
	local v_owner = self.tool:getOwner()
...
function DP27:client_onEquip(animate, is_custom)
...
function DP27:sv_n_onAim(aiming)
	self.network:sendToClients( "cl_n_onAim", aiming )
end

function DP27:cl_n_onAim(aiming)
	if not self.cl_isLocal and self.tool:isEquipped() then
		self:onAim(aiming)
	end
end

function DP27:onAim(aiming)
	self.aiming = aiming
...
function DP27:calculateTpMuzzlePos()
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function DP27:calculateFpMuzzlePos()
...
function DP27:sv_n_onReload(anim_id)
	self.network:sendToClients("cl_n_onReload", anim_id)
end

function DP27:cl_n_onReload(anim_id)
	if not self.cl_isLocal and self.tool:isEquipped() then
		self:cl_startReloadAnim(id_to_anim_name[anim_id])
	end
end

function DP27:cl_startReloadAnim(anim_name)
...
function DP27:sv_n_checkMag()
	self.network:sendToClients("cl_n_checkMag")
end

function DP27:cl_n_checkMag()
	local s_tool = self.tool
...
function DP27:cl_startCheckMagAnim()
...
function DP27:client_onToggle()
	if not self:client_isGunReloading() and not self.aiming and not self.tool:isSprinting() and self.fireCooldownTimer == 0.0 and self.equipped then
		if self.ammo_in_mag > 0 then
			...
			sm.gui.displayAlertText("DP27: No Ammo. Reloading...", 3)
