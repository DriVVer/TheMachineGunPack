...
local renderables =
{
	...
	"$CONTENT_DATA/Tools/Renderables/PPSh/PPSh_AnimDrum.rend"
}
...
function PPShDrum:client_initAimVals()
	local cameraWeight, cameraFPWeight = self.tool:getCameraWeights()
	self.aimWeight = math.max( cameraWeight, cameraFPWeight )
end

function PPShDrum:server_onCreate()
...
function PPShDrum:client_onCreate()
	...
	mgp_toolAnimator_initialize(self, "ppsh_drum")

	self.network:sendToServer("server_requestAmmo")
end

function PPShDrum.client_onDestroy(self)
	mgp_toolAnimator_destroy(self)
end

function PPShDrum.client_onRefresh( self )
	self:loadAnimations()
end

function PPShDrum.loadAnimations( self )
	...
	if self.cl_isLocal then
		self.fpAnimations = createFpAnimations(
			self.tool,
			{
				equip = { "PPShDrum_pickup", { nextAnimation = "idle" } },
				unequip = { "PPShDrum_putdown" },

				idle = { "PPShDrum_idle", { looping = true } },
				shoot = { "PPShDrum_shoot", { nextAnimation = "idle" } },

				reload = { "PPShDrum_reload", { nextAnimation = "idle", duration = 1.0 } },
				reload_empty = { "PPShDrum_reload_empty", { nextAnimation = "idle", duration = 1.0 } },

				ammo_check = { "PPShDrum_ammo_check", { nextAnimation = "idle", duration = 1.0 } },

				aimInto = { "PPShDrum_aim_into", { nextAnimation = "aimIdle" } },
				aimExit = { "PPShDrum_aim_exit", { nextAnimation = "idle", blendNext = 0 } },
				aimIdle = { "PPShDrum_aim_idle", { looping = true } },
				aimShoot = { "PPShDrum_aim_shoot", { nextAnimation = "aimIdle"} },

				sprintInto = { "PPShDrum_sprint_into", { nextAnimation = "sprintIdle",  blendNext = 0.2 } },
				sprintExit = { "PPShDrum_sprint_exit", { nextAnimation = "idle",  blendNext = 0 } },
				sprintIdle = { "PPShDrum_sprint_idle", { looping = true } },
			}
		)
	end
...
function PPShDrum:client_updateAimWeights(dt)
	-- Camera update
	local bobbing = 1
	if self.aiming then
...
function PPShDrum:server_spendAmmo(data, player)
...
function PPShDrum:sv_n_trySpendAmmo(data, player)
	local v_owner = self.tool:getOwner()
...
function PPShDrum.client_onUpdate( self, dt )
...
function PPShDrum:client_onEquip(animate, is_custom)
...
function PPShDrum:client_onUnequip(animate, is_custom)
	if not is_custom and TSU_IsOwnerSwimming(self) then
		return
	end
...
function PPShDrum:sv_n_onAim(aiming)
...
function PPShDrum:onAim(aiming)
	self.aiming = aiming
...
function PPShDrum:sv_n_onShoot(dir)
...
function PPShDrum:calculateFirePosition()
...
function PPShDrum:calculateTpMuzzlePos()
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function PPShDrum:calculateFpMuzzlePos()
...
function PPShDrum:cl_onPrimaryUse(is_shooting)
...
function PPShDrum:sv_n_onReload(anim_id)
...
function PPShDrum:cl_initReloadAnim(anim_name)
...
function PPShDrum:client_onReload()
	if self.equipped then
...
function PPShDrum:sv_n_checkMag()
	self.network:sendToClients("cl_n_checkMag")
end

function PPShDrum:cl_n_checkMag()
	local s_tool = self.tool
...
function PPShDrum:cl_startCheckMagAnim()
	setTpAnimation(self.tpAnimations, "ammo_check", 1.0)
end

function PPShDrum:client_onToggle()
	if not self:client_isGunReloading() and not self.aiming and not self.tool:isSprinting() and self.fireCooldownTimer == 0.0 and self.equipped then
		if self.ammo_in_mag > 0 then
			sm.gui.displayAlertText(("PPShDrum: Ammo #ffff00%s#ffffff/#ffff00%s#ffffff"):format(self.ammo_in_mag, self.mag_capacity), 2)
			setFpAnimation(self.fpAnimations, "ammo_check", 0.0)

			self:cl_startCheckMagAnim()
			self.network:sendToServer("sv_n_checkMag")
		else
			sm.gui.displayAlertText("PPShDrum: No Ammo. Reloading...", 3)
...
function PPShDrum.cl_onSecondaryUse( self, state )
